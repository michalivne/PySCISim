...
, m_points( points )
, m_r( r )
, m_sphere_renderer( 1.0, num_subdivs )
, m_cylinder_verts()
, m_cylinder_normals()
{
...
void StapleRenderer::renderBody( const Eigen::Matrix<GLfloat,3,1>& color )
{
  ...
  GLfloat mcolorambient[] = { (GLfloat) 0.3 * color.x(), (GLfloat) 0.3 * color.y(), (GLfloat) 0.3 * color.z(), (GLfloat) 1.0 };
  glMaterialfv( GL_FRONT_AND_BACK, GL_AMBIENT, mcolorambient );
  GLfloat mcolordiffuse[] = { color.x(), color.y(), color.z(), (GLfloat) 1.0 };
...
{
  int num_samples = 4;
  for( int i = 0; i < num_subdivs; ++i ) num_samples *= 2;
  return num_samples;
}
...
void StapleRenderer::initializeCylinderMemory()
{
  ...
  const GLfloat dtheta = 2.0 * PI / ((GLfloat)m_num_samples);

  for( int quad_num = 0; quad_num < m_num_samples; ++ quad_num )
  {
    const GLfloat c0 = m_r * cos( quad_num * dtheta );
    const GLfloat s0 = m_r * sin( quad_num * dtheta );
    const GLfloat c1 = m_r * cos( ((quad_num+1)%m_num_samples) * dtheta );
    const GLfloat s1 = m_r * sin( ((quad_num+1)%m_num_samples) * dtheta );
